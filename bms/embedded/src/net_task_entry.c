...
const char vc_cloud_1[] =
    "{\"@context\":[\"https://www.w3.org/2018/credentials/v1\",\"http://"
    "localhost:8443/docs/"
    "vc.serviceAccess.schema.html\"],\"id\":\"urn:uuid:a1fd774e-5300-4171-b778-"
    "e53cedb64823\",\"type\":[\"VerifiableCredential\",\"CloudInstance\"],"
    "\"issuer\":\"did:batterypass:bms.sn-544b51e7\",\"holder\":\"did:"
    "batterypass:cloud.sn-central\",\"issuanceDate\":\"2025-06-17T16:17:43Z\","
    "\"expirationDate\":\"2026-06-17T16:17:43Z\",\"credentialSubject\":{\"id\":"
    "\"did:batterypass:cloud.sn-cloud1\",\"type\":\"CloudInstance\","
    "\"cloudDid\":\"did:batterypass:cloud.sn-cloud1\",\"timestamp\":\"2026-06-"
    "17T16:17:43Z\"},\"proof\":{\"type\":\"EcdsaSecp256r1Signature2019\","
    "\"created\":\"2025-06-17T16:17:43Z\",\"verificationMethod\":\"did:"
    "batterypass:bms.sn-544b51e7#key-1\",\"proofPurpose\":\"authentication\","
    "\"jws\":\"\"}}";
const char *vcs[] = {vc_cloud_1};

__attribute__((section(".data_flash"))) const int number_of_vcs = 1;
  ...
  while (1) {
    ...
    for (uint8_t i = 0; i < number_of_vcs; i++) {
      ...
      sprintf(http_request,
              "GET /api/v1/dids/%s HTTP/1.1\r\nHost: "
              "localhost:8443\r\nUser-Agent: BMS\r\nAccept: */*\r\n\r\n",
              did_request);
      int len_request = strlen(http_request);
      int status = 0;
      // Get all endpoints via the did request from the VC
      status = vTCPSendWithRetries(BLOCKCHAIN_ENDPOINT, 8080, http_request,
                                   len_request, response, NETWORK_RETRIES);
      if (status) {
        continue;
      }
      ...
      xMessageBufferSend(net_crypto_message_buffer, (void *)did_pointer,
                         strlen(did_pointer), pdMS_TO_TICKS(1000));
    }
    for (uint8_t i = 0; i < number_of_vcs; i++) {
      sending_and_receiving_functionality();
    }
...
int sending_and_receiving_functionality() {
  ...
  do {
    endpoint_dns_length =
        xMessageBufferReceive(crypto_net_message_buffer, (void *)endpoint_dns,
                              MAX_ENDPOINT_DNS_SIZE, pdMS_TO_TICKS(500));
  ...
  if (endpoint_reachable) {
    xMessageBufferSend(net_crypto_message_buffer, (void *)&ack, ACK_LENGTH,
                       pdMS_TO_TICKS(1000));
    char cReceivedString[600] = {RESET_VALUE};
    size_t xReceivedBytes = RESET_VALUE;
    do {
      xReceivedBytes = xMessageBufferReceive(
          crypto_net_message_buffer, (void *)cReceivedString,
          sizeof(cReceivedString), pdMS_TO_TICKS(1000));
    ...
    sprintf(http_request,
            "POST /batterypass/%s HTTP/1.1\r\nHost: "
            "localhost:8000\r\nUser-Agent: BMS\r\nAccept: */*\r\n\r\n%s",
            did, cReceivedString);
    int status =
        vTCPSendWithRetries(gp_remote_ip_address, 8000, http_request,
                            strlen(http_request), NULL, NETWORK_RETRIES);
    vPortFree(did);
  }
}
...
int vTCPSend(const char *pcIPAddress, uint16_t pcPort, char *pcBufferToTransmit,
             const size_t xTotalLengthToSend, char *pcBufferToReceive) {
  ...
  Socket_t xSocket = FreeRTOS_socket(FREERTOS_AF_INET4, FREERTOS_SOCK_STREAM,
  ...
  if (FreeRTOS_connect(xSocket, &xRemoteAddress, sizeof(xRemoteAddress)) == 0) {
    ...
    while (xAlreadyTransmitted < xTotalLengthToSend) {
      ...
      /* This RTOS task is going to send using the zero copy interface.  The
          ...
          buffer that is passed into, rather than copied into, the
         FreeRTOS_send() function. */
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       * @retval     Status
                                                                                                                       **********************************************************************************************************************/
int vTCPSendWithRetries(const char *pcIPAddress, uint16_t pcPort,
                        char *pcBufferToTransmit,
                        const size_t xTotalLengthToSend,
                        char *pcBufferToReceive, int retries) {
  for (int i = 0; i < retries; i++) {
    int status = vTCPSend(pcIPAddress, pcPort, pcBufferToTransmit,
                          xTotalLengthToSend, pcBufferToReceive);
    if (status != 0) {
      return 0;
    }
  }
...
BaseType_t vSendPing(const char *pcIPAddress) {
   ...
   * decimal dot notation (for example, “192.168.0.200”). Convert the string
   * into the required 32-bit format.
   */
...
void print_ipconfig() {
  ...
  APP_PRINT("\tDefault Gateway . . . . . . . . . : %d.%d.%d.%d\r\n",
            ucGatewayAddress[0], ucGatewayAddress[1], ucGatewayAddress[2],
            ucGatewayAddress[3]);
  APP_PRINT("\tDNS Servers . . . . . . . . . . . : %d.%d.%d.%d\r\n",
            ucDNSServerAddress[0], ucDNSServerAddress[1], ucDNSServerAddress[2],
