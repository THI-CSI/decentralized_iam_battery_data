...
type MyServer struct {
	...
	requestVcCreateSchemaCloudInstance *gojsonschema.Schema
...
func (s *MyServer) validateIncomingRequest(ctx echo.Context, data interface{}, schema *gojsonschema.Schema) error {
	...
	if !validationResult.Valid() {
		...
		return echo.NewHTTPError(http.StatusBadRequest, models.ResponseErrorSchema{
...
func (s *MyServer) validateOutgoingResponse(ctx echo.Context, data interface{}, schema *gojsonschema.Schema) error {
	...
	if validateErr != nil {
		...
		return echo.NewHTTPError(http.StatusInternalServerError, validateErr.Error()) // Message is string
	}
	...
	if !validationResult.Valid() {
		...
		return echo.NewHTTPError(http.StatusInternalServerError, models.ResponseErrorSchema{
			Message: msg,                // Message is string
			Details: &validationDetails, // Details is *[]struct, assign address of slice
		})
	}
