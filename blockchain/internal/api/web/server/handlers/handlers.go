...
func (s *MyServer) validateIncomingRequest(ctx echo.Context, data interface{}, schema *gojsonschema.Schema, logIdentifier string) error {
	documentLoader := gojsonschema.NewGoLoader(data)

	validationResult, validateErr := schema.Validate(documentLoader)
	if validateErr != nil {
		log.Printf("Internal error during %s schema validation: %v", logIdentifier, validateErr)
	...
	if !validationResult.Valid() {
		...
		log.Printf("Bad Request: %s did NOT conform to schema! Errors: %v", logIdentifier, validationDetails)
...
func (s *MyServer) validateOutgoingResponse(ctx echo.Context, data interface{}, schema *gojsonschema.Schema, logIdentifier string, problemIdentifier string, problemID int) error {
	documentLoader := gojsonschema.NewGoLoader(data)

	validationResult, validateErr := schema.Validate(documentLoader)
	if validateErr != nil {
		log.Printf("Internal error during %s schema validation (%s %d): %v", logIdentifier, problemIdentifier, problemID, validateErr)
	...
	if !validationResult.Valid() {
		...
		log.Printf("CRITICAL: %s for %s %d do NOT conform to schema! Errors: %v", logIdentifier, problemIdentifier, problemID, validationDetails)

		msg := fmt.Sprintf("Internal server error: Malformed %s data generated by server", logIdentifier)
